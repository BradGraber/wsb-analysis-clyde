...
## Acceptance Criteria
- [ ] Fetches up to 1000 comments per post using Async PRAW, sorted by engagement metric (score x replies)
...
- [ ] Comments from deleted or removed authors are still fetched (author set to "[deleted]" as returned by Async PRAW)
...
## Technical Notes
- Async PRAW's comment forest provides threaded structure; use replace_more() to expand collapsed comment threads before iteration

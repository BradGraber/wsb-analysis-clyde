...
## Technical Notes
- reply_count = number of direct replies to the comment (from PRAW comment.replies length, captured during fetch)
- Normalization must happen AFTER all comments are scored (two-pass: compute raw, then normalize)
- The reply_count should be captured during comment fetching (task-002-003-01) as an additional field or computed here from PRAW data

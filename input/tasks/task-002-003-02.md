...
## Description
Add a `build_parent_chains(comments: list[ProcessedComment], comment_forest) -> None` function that populates the parent_chain field on each ProcessedComment by walking up the PRAW comment tree from each comment to the root, collecting ParentChainEntry objects (id, body, depth, author) in order from immediate parent to root. Top-level comments get an empty parent_chain.
...
- [ ] Deleted parent comments use "[deleted]" for author and body fields as returned by PRAW
...
## Technical Notes
- PRAW's comment.parent() returns the parent Comment or Submission; walk until reaching a Submission (the post)
